...
function createMatIdentity(module: EmscriptenModule): Transform {
...
export abstract class Tool {
  ...
  constructor(type: ToolType, module: EmscriptenModule) {
    this.mType = type;
    Tool.MAT_IDENTITY = createMatIdentity(module);
    this.mColor = new module.ParticleColor();
    this.mVelocity = new module.Vec2(0, 0);
    this.mShape = new module.CircleShape();
    this.mModule = module;
  }
  ...
  public onTouch(v: HTMLCanvasElement, e: MotionEvent): void {
    switch (e.getActionMasked()) {
      ...
      case MotionEvent.ACTION_POINTER_DOWN: {
        ...
        const pInfo: PointerInfo = new PointerInfo(pID, this.mModule);
        console.assert(
          Tool.mGroupMap.get(pID) == null,
          'mGroupMap with same ID already exist.',
        );
        this.processTouchInput(v, e, pInfo, pointerIndex, true, true);
      ...
      case MotionEvent.ACTION_MOVE: {
        for (
          ...
          console.assert(pInfo != null, 'Expected pInfo not to be null');
          this.processTouchInput(v, e, pInfo, pointerIndex, true, true);
          // Put updated PointerInfo back in map
          Tool.mGroupMap.set(pID, pInfo);
        }
  ...
  private applyToolAcrossRange(
    ...
    const viewHeight = v.getBoundingClientRect().height; // worldHeight;
    const viewWidth = v.getBoundingClientRect().width; // worldWidth;
  ...
  protected applyTool(pInfo: PointerInfo): void {
    ...
    if (this.mOperations.has(ToolOperation.ADD_PARTICLES)) {
      pgd = new this.mModule.ParticleGroupDef();
