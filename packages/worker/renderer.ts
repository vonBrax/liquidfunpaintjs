...
export class Renderer {
  ...
  public static PARTICLE_RADIUS = 0.03;
  ...
  private mInnerBoundaryBody: LiquidFun.Body = null;
  ...
  public init(module: EmscriptenModule): void {
    this.mModule = module;
    ...
    this.mParticleRenderer = new ParticleRenderer(module);

    if (Renderer.DEBUG_DRAW) {
      const dr = new DebugRenderer(module);
  ...
  public onSurfaceChanged(
    ...
    console.log({
      worldHeight: this.sRenderWorldHeight,
      worldWidth: this.sRenderWorldWidth,
      screenHeight: height,
      screenWidth: width,
    });
  ...
  public async onSurfaceCreated(): Promise<void> {
    ...
    TextureRenderer.getInstance(this.mModule).onSurfaceCreated();
  ...
  public reset(): void {
    ...
    try {
      this.deleteWorld();
      const gravity = new this.mModule.Vec2(0, 0);
      this.mWorld = new this.mModule.World(gravity);
  ...
  private initParticleSystem(): void {
    ...
    try {
      ...
      const psDef: LiquidFun.ParticleSystemDef = new this.mModule.ParticleSystemDef();
  ...
  createGlass(): void {
    const world = this.acquireWorld();
    // clean up previous Body if exists
    if (this.mInnerBoundaryBody != null) {
      world.DestroyBody(this.mInnerBoundaryBody);
    }
    const Module = this.mModule;
    const bodyDef = new Module.BodyDef();
    const boundaryPolygon = new Module.PolygonShape();
    this.mInnerBoundaryBody = world.CreateBody(bodyDef);
    const glassBody = this.mInnerBoundaryBody;
    const glassHeight = 0.8 * this.sRenderWorldHeight;
    const glassWidth = glassHeight / 3;
    const glassThickness = glassHeight / 20;
    // BOTTOM
    console.log('10:36');
    const centerBottom = new Module.Vec2(
      this.sRenderWorldWidth / 2,
    ...
    boundaryPolygon.SetAsBox(glassWidth, 0.5, centerBottom, 0);
    glassBody.CreateFixture(boundaryPolygon, 0.0);
    // LEFT
    // const centerLeft = new Module.Vec2(-glassWidth / 2, -glassHeight / 2);
    // boundaryPolygon.SetAsBox(glassThickness, glassHeight, centerLeft, 0);
    // glassBody.CreateFixture(boundaryPolygon, 0.0);
    // RIGHT
    // const centerRight = new Module.Vec2(glassWidth / 2, -glassHeight / 2);
    // boundaryPolygon.SetAsBox(glassThickness, glassHeight, centerRight, 0);
    // glassBody.CreateFixture(boundaryPolygon, 0.0);
    bodyDef.delete();
    boundaryPolygon.delete();
  }
