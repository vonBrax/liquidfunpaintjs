...
function createByteBuffer(
  ...
  const buffer = new ByteBuffer(size, module);
  buffer.nativeOrder();
...
export class RenderHelper {
  ...
  // );
  private static _screenQuadVertexBuffer: ByteBuffer;

  public static SCREEN_QUAD_VERTEX_BUFFER(
    module: EmscriptenModule,
  ): ByteBuffer {
    if (!RenderHelper._screenQuadVertexBuffer) {
      RenderHelper._screenQuadVertexBuffer = createByteBuffer(
        RenderHelper.SCREEN_QUAD_VERTEX_DATA.length * 4,
        module,
        RenderHelper.SCREEN_QUAD_VERTEX_DATA,
      );
    }
    return RenderHelper._screenQuadVertexBuffer;
  }
}
