...
import { ShaderProgram, Material, AttributeInfo, Texture } from './shader';
...
export class DebugRenderer {
  ...
  constructor(private mModule: EmscriptenModule) {
    ...
    // this.b2Draw = newDraw;
    // console.log(newDraw);
    this.mTransformFromWorld = mat4.create();
    // Make sure we set the right context when
    // function is called from c++
    this.bindMethods();

    // Adjust buffer capacity to account for
    // max number of particles in Renderer
    DebugRenderer.DEBUG_CAPACITY = Math.max(
      DebugRenderer.DEBUG_CAPACITY,
      Renderer.MAX_PARTICLE_COUNT * 32,
    );

    // this.mPolygonPositionBuffer = gl.createBuffer();
    this.mPolygonPositionBuffer = new ByteBuffer(
      DebugRenderer.DEBUG_CAPACITY,
      mModule,
    );
    ...
    this.mPolygonColorBuffer = new ByteBuffer(
      DebugRenderer.DEBUG_CAPACITY,
      mModule,
    );
    ...
    this.mCirclePositionBuffer = new ByteBuffer(
      DebugRenderer.DEBUG_CAPACITY,
      mModule,
    );
    ...
    this.mCircleColorBuffer = new ByteBuffer(
      DebugRenderer.DEBUG_CAPACITY,
      mModule,
    );
    ...
    this.mCirclePointSizeBuffer = new ByteBuffer(
      DebugRenderer.DEBUG_CAPACITY,
      mModule,
    );
    ...
    this.mLinePositionBuffer = new ByteBuffer(
      DebugRenderer.DEBUG_CAPACITY,
      mModule,
    );
    ...
    this.mLineColorBuffer = new ByteBuffer(
      DebugRenderer.DEBUG_CAPACITY,
      mModule,
    );
